    ...
    checkArgs2(msg, msgArgs) {
        ...
        if (this.args != undefined) {
            ...
            for (let cmdArg of this.args) {
                ...
                // Its not a optional argument
                if (!cmdArg.optional) {
                    // There are not more message argumensts
                    if (!msgArgs[index]) {
                        util.send(msg, cmdArg.invalidError);
                        return false;
                    }
                    // There is a message argument
                    else {
                        // If Arg.checkArg() true
                        if (cmdArg.checkArg(msg, msgArgs[index])) {
                            index++;
                            continue;
                        }
                        // If Arg.checkArg() false
                        else {
                            util.send(msg, cmdArg.invalidError);
                            return false;
                        }
                ...
                // Its a optional argument
                else {
                    // There are not more message argumensts
                    if (!msgArgs[index]) {
                        // Need to continue to check if have more not optional argument
                        continue;
                    }
                    // There is a message argument
                    else {
